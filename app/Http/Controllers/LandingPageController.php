...
class LandingPageController extends Controller
{
    public function index()
    {
        ...
        // Key dates: the college's own (set by MIS under settings); otherwise a
        // sensible default timeline.
        $calendar = collect($college->key_dates ?? [])
            ->map(fn ($d) => ['title' => $d['title'] ?? '', 'date' => $d['date'] ?? ''])
            ->filter(fn ($d) => $d['title'] !== '' && $d['date'] !== '')
            ->values()->all();
        ...
        // Provost block comes from the college record (per-tenant; MIS-editable).
        $provost = [
            'name'    => $college->provost_name ?? 'The Provost',
            'title'   => $college->provost_title ?? 'Provost',
            'photo'   => $college->provost_photo,
