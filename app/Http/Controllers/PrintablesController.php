...
class PrintablesController extends Controller
{
    ...
    /** JSON: a student's paid receipts + outstanding invoices, newest first. */
    public function studentReceipts(Student $student)
    {
        ...
        return response()->json([
            ...
            'receipts' => $invoices->where('status', 'paid')->values()->map($map),
            'unpaid'   => $invoices->where('status', '!=', 'paid')->values()->map($map),
        ]);
    }
