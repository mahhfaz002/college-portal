<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Payroll Review — Provost</h2>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border overflow-x-auto">
                <div class="px-6 py-4 bg-gray-50 border-b font-bold text-gray-700">
                    Payslips for {{ \Illuminate\Support\Carbon::parse($month.'-01')->format('F Y') }} ({{ $slips->count() }})
                </div>
                <table class="w-full text-left text-sm">
                    <thead><tr class="bg-gray-50 border-b text-xs uppercase text-gray-500">
                        <th class="p-3">Staff</th><th class="p-3 text-right">Net Salary</th><th class="p-3">Status</th><th class="p-3">Proprietor Note</th><th class="p-3 text-right">Action</th>
                    </tr></thead>
                    <tbody>
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-3">
                                <span class="text-[10px] uppercase font-bold px-2 py-1 rounded
                                    {{ $slip->status === 'approved' ? 'bg-green-100 text-green-700' : ($slip->status === 'paid' ? 'bg-emerald-100 text-emerald-700' : ($slip->status === 'queried' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700')) }}">
                            ...
                            <td class="p-3 text-right whitespace-nowrap">
                                @if($slip->status === 'provost_review')
                                    ...
                                    <form method="POST" action="{{ route('payroll.provost.query', $slip) }}" class="inline ml-1">@csrf
                                ...
                                @elseif($slip->proprietor_status === 'queried' && $slip->status === 'provost_forwarded')
                                    <form method="POST" action="{{ route('payroll.provost.relay', $slip) }}" class="inline" onsubmit="return confirm('Relay the Proprietor query down to the Bursar?')">@csrf
                                        <button class="bg-amber-600 text-white text-xs px-3 py-1.5 rounded font-bold hover:bg-amber-700">Send to Bursar</button>
                                    </form>
                                @else
                        ...
                        @empty
                        <tr><td colspan="5" class="p-8 text-center text-gray-400">No payslips submitted for review.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
